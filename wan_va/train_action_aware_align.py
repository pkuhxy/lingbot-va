...
"""Action-aware transition alignment for joint video/action flow matching.

The original auxiliary objective subtracts final hidden states produced at
independently sampled diffusion timesteps and uses one-to-one InfoNCE.  That
mixes temporal change with denoising-state change and treats similar actions as
false negatives.  This trainer instead:

1. reconstructs the model's predicted clean video latent (x0) from its normal
   flow-matching output and forms spatially pooled x0[t+1] - x0[t] features;
2. predicts the complete action chunk rather than a token-averaged action;
3. distils an action-similarity distribution, so similar actions are soft
   positives instead of mutually exclusive negatives; and
4. warms up and caps the auxiliary objective relative to the primary loss.

Only the video prediction path receives auxiliary backbone gradients.  Clean
actions are detached targets, which removes the moving/noisy action-hidden
shortcut while preserving the original video and action flow-matching losses.
"""
...
class ActionAwareTransitionHead(nn.Module):
    """Predicts a full action chunk from a denoised video transition."""
    ...
    def forward(self, video_features):
        transition_embedding = self.video_encoder(video_features.float())
        action_prediction = self.action_decoder(transition_embedding)
        return transition_embedding, action_prediction
...
class ActionAwareAlignTrainer(Trainer):
    def __init__(self, config):
        ...
        self.action_aware_head = ActionAwareTransitionHead(
    ...
    def _load_action_aware_head(self, checkpoint_path):
        if not checkpoint_path:
            return
        head_file = Path(checkpoint_path) / "action_aware_head" / "model.safetensors"
    ...
    def _soft_contrastive_loss(self, action_prediction, action_target, action_mask):
        mask = action_mask.float()
        predicted = F.normalize(action_prediction.float() * mask, dim=-1, eps=1e-6)
        target = F.normalize(action_target.float() * mask, dim=-1, eps=1e-6)
        candidate_target, positive_offset = self._gather_detached(target)

        if candidate_target.shape[0] < 2:
            zero = action_prediction.new_zeros(())
        ...
        student_logits = predicted @ candidate_target.t() / self.student_temperature
        loss = -(target_distribution * F.log_softmax(student_logits, dim=-1)).sum(-1).mean()

        with torch.no_grad():
            top1 = (student_logits.argmax(dim=-1) == positive_ids).float().mean()
            soft_positive_mass = (
                target_distribution.sum(dim=-1)
                - target_distribution[local_ids, positive_ids]
            ).mean()
    ...
    def _compute_action_aware_losses(self, input_dict, latent_prediction):
        ...
        _, action_prediction = self.action_aware_head(video_features)
        ...
        soft_loss, top1, soft_positive_mass = self._soft_contrastive_loss(
            action_prediction,
            action_target,
            action_mask,
        )
    ...
    def save_checkpoint(self):
        ...
        if self.config.rank == 0:
            ...
            head_dir = checkpoint_dir / "action_aware_head"
    ...
    def train(self):
        logger.info("Starting action-aware alignment training for %s steps", self.config.num_steps)
        ...
        progress.close()
        logger.info("Action-aware alignment training completed")
...
def main():
    parser = argparse.ArgumentParser(description="Train action-aware video transitions")
