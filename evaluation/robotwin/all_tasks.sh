...
save_root=${1:-'/mnt/data/users/xianyi/EmbodyAi/lingbot-va/results_base'}
seed=${2:-0}
test_num=${3:-1}
...
log_dir="./logs/all_tasks"
mkdir -p "$log_dir"

batch_time=$(date +%Y%m%d_%H%M%S)
pid_file="pids_all_tasks_${batch_time}.txt"
> "$pid_file"
...
for (( batch_start=0; batch_start<total; batch_start+=num_gpus )); do
    ...
    echo -e "\033[32mLaunching batch $(( batch_start / num_gpus + 1 )): tasks ${batch_start}..$(( batch_end - 1 ))\033[0m"
    pids=()
    ...
    for (( i=batch_start; i<batch_end; i++ )); do
        ...
        log_file="${log_dir}/${i}_${task_name}_${batch_time}.log"

        echo -e "\033[33m[Task $i/$(( total - 1 ))] ${task_name}: GPU ${gpu_id}, PORT ${port}, Log ${log_file}\033[0m"
        run_one_task "$i" "$task_name" "$gpu_id" "$port" "$log_file" &
        pid=$!
        pids+=("$pid")
        echo "${pid}" >> "$pid_file"
    done
    ...
    if (( failed )); then
        echo "At least one task failed in this batch. See logs under ${log_dir}." >&2
        exit 1
    fi
done

echo -e "\033[32mAll ${total} tasks completed. Each task ran test_num=${test_num} episodes.\033[0m"
