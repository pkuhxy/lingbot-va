...
SAVE_ROOT="${SAVE_ROOT:-${ROOT}/ckpts/action_aware_mutual_transition_bs8_f4_${RUN_TAG}}"
...
if [[ "${ENABLE_WANDB}" == "1" ]]; then
    ...
    export WANDB_PROJECT="${WANDB_PROJECT:-va_robotwin_action_aware_mutual}"
    export WANDB_RUN_NAME="${WANDB_RUN_NAME:-action_aware_mutual_bs8_f4_${RUN_TAG}}"
...
mkdir -p "${SAVE_ROOT}"
echo "Mutual action-aware training: GPUs=${NGPU}, per_gpu_batch=${BATCH_SIZE:-8}, latent_frames=4"
...
"${PYTHON}" -m torch.distributed.run \
    ...
    --tee=3 \
    -m wan_va.train_action_aware_mutual_align \
    "${args[@]}"
